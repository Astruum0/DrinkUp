...
const CocktailPreviewCard = ({
  ...
  return (
    <div className="cocktail-preview-card">
      <img
        className="cocktail-image-preview"
      ...
      {fullyDetailed && (
        <>
          <p className="description">
            {cocktail.description ? cocktail.description : "Aucune description"}
          </p>
          <ul className="recipe">
            {cocktail.ingredients
              .filter((r) =>
                ownedIngredients?.map((i) => i.id).includes(r.ingredient.id)
              )
              .map((r) => {
                return (
                  <li key={r.ingredient.id} className="owned">
                    – {r.ingredient.name} {r.quantity ? `(${r.quantity})` : ""}{" "}
